...
public class FormationContinueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all formationContinues")
                    .get("/api/formation-continues")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formationContinue")
                        .post("/api/formation-continues")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomFormationC\": \"SAMPLE_TEXT\"" +
                                ", \"duree\": \"SAMPLE_TEXT\"" +
                                ", \"admission\": \"CONCOURS\"" +
                                ...
                                ", \"lycee\": \"LTID_DAKAR\"" +
                                ", \"ficheFormation\": null" +
                                ", \"libellePC\": \"SAMPLE_TEXT\"" +
                                ", \"montantPriseEnCharge\": 0" +
                                ", \"coutFormation\": 0" +
                                ", \"detailPC\": null" +
                                ", \"nomDiplome\": \"CPS\"" +
                                ", \"autreDiplome\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formationContinue_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formationContinue").get("${new_formationContinue_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created formationContinue").delete("${new_formationContinue_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormationContinue entity").exec(scn);
