...
public class ServiceMFPAIGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all serviceMFPAIS").get("/api/service-mfpais").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new serviceMFPAI")
                        .post("/api/service-mfpais")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"imageService\": null" +
                                ", \"nomService\": \"SAMPLE_TEXT\"" +
                                ", \"chefService\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_serviceMFPAI_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created serviceMFPAI").get("${new_serviceMFPAI_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created serviceMFPAI").delete("${new_serviceMFPAI_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ServiceMFPAI entity").exec(scn);
