...
public class BailleurGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all bailleurs").get("/api/bailleurs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bailleur")
                        .post("/api/bailleurs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomBailleur\": \"SAMPLE_TEXT\"" +
                                ", \"budgetPrevu\": 0" +
                                ", \"budgetDepense\": 0" +
                                ", \"budgetRestant\": 0" +
                                ", \"nbrePC\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_bailleur_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created bailleur").get("${new_bailleur_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created bailleur").delete("${new_bailleur_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Bailleur entity").exec(scn);
