 ...
 */
public class DossierGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dossiers").get("/api/dossiers").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dossier")
                        .post("/api/dossiers")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"numDossier\": \"SAMPLE_TEXT\"" +
                                ", \"prenom\": \"SAMPLE_TEXT\"" +
                                ", \"nom\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"regionNaiss\": \"DAKAR\"" +
                                ", \"departementNaiss\": \"DAKAR\"" +
                                ", \"typePiece\": \"CNI\"" +
                                ", \"numeroPiece\": 0" +
                                ", \"sexe\": \"HOMME\"" +
                                ", \"regionResidence\": \"DAKAR\"" +
                                ", \"depResidence\": \"DAKAR\"" +
                                ", \"adresseResidence\": \"SAMPLE_TEXT\"" +
                                ", \"telephone1\": \"SAMPLE_TEXT\"" +
                                ", \"telephone2\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                                ", \"niveauFormation\": \"CINQUIEME\"" +
                                ", \"specialite\": \"AGRI_ELEVAGE\"" +
                                ", \"intituleDiplome\": \"SAMPLE_TEXT\"" +
                                ", \"diplome\": null" +
                                ", \"anneeObtention\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"lieuObtention\": \"SAMPLE_TEXT\"" +
                                ", \"cv\": null" +
                                ", \"lettreMotivation\": null" +
                                ", \"profession\": \"SAMPLE_TEXT\"" +
                                ", \"autreSpecialite\": \"SAMPLE_TEXT\"" +
                                ", \"nomCompetence\": \"SAMPLE_TEXT\"" +
                                ", \"niveauCompetence\": \"DEBUTANT\"" +
                                ", \"intituleExperience\": \"SAMPLE_TEXT\"" +
                                ", \"posteOccupe\": \"SAMPLE_TEXT\"" +
                                ", \"dateDebut\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFin\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"nomEntreprise\": \"SAMPLE_TEXT\"" +
                                ", \"mission\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dossier_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dossier").get("${new_dossier_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created dossier").delete("${new_dossier_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Dossier entity").exec(scn);
