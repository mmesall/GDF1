...
public class PriseEnChargeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all priseEnCharges").get("/api/prise-en-charges").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new priseEnCharge")
                        .post("/api/prise-en-charges")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelle\": \"SAMPLE_TEXT\"" + ", \"montantPC\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_priseEnCharge_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created priseEnCharge").get("${new_priseEnCharge_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created priseEnCharge").delete("${new_priseEnCharge_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PriseEnCharge entity").exec(scn);
