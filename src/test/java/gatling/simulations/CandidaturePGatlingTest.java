...
public class CandidaturePGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all candidaturePS").get("/api/candidature-ps").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new candidatureP")
                        .post("/api/candidature-ps")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_candidatureP_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created candidatureP").get("${new_candidatureP_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created candidatureP").delete("${new_candidatureP_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CandidatureP entity").exec(scn);
