...
public class FormationInitialeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all formationInitiales")
                    .get("/api/formation-initiales")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formationInitiale")
                        .post("/api/formation-initiales")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomFormationI\": \"SAMPLE_TEXT\"" +
                                ", \"duree\": \"SAMPLE_TEXT\"" +
                                ", \"admission\": \"CONCOURS\"" +
                                ", \"diplomeRequis\": \"ATTESTATION\"" +
                                ", \"niveauEtude\": \"CINQUIEME\"" +
                                ", \"ficheFormation\": null" +
                                ", \"filiere\": \"AGRI_ELEVAGE\"" +
                                ", \"serie\": \"STEG\"" +
                                ", \"cfp\": \"CEDT_G15\"" +
                                ", \"lycee\": \"LTID_DAKAR\"" +
                                ", \"nomConcours\": \"SAMPLE_TEXT\"" +
                                ", \"dateOuverture\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateCloture\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateConcours\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"nomDiplome\": \"CPS\"" +
                                ", \"nomDebouche\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formationInitiale_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formationInitiale").get("${new_formationInitiale_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created formationInitiale").delete("${new_formationInitiale_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormationInitiale entity").exec(scn);
