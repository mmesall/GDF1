...
public class EtudiantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all etudiants").get("/api/etudiants").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created etudiant").get("${new_etudiant_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created etudiant").delete("${new_etudiant_url}").headers(headers_http_authenticated))
                .pause(10)
        );
