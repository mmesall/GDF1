...
public class CandidatureEGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all candidatureES").get("/api/candidature-es").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new candidatureE")
                        .post("/api/candidature-es")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"offreFormation\": \"AGRI_ELEVAGE\"" +
                                ", \"dateDebutOffre\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFinOffre\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateDepot\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"resultat\": \"SOUMIS\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_candidatureE_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created candidatureE").get("${new_candidatureE_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created candidatureE").delete("${new_candidatureE_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CandidatureE entity").exec(scn);
