...
public class BailleurResource {
    ...
    public ResponseEntity<Bailleur> createBailleur(@Valid @RequestBody Bailleur bailleur) throws URISyntaxException {
        ...
        bailleur = bailleurService.save(bailleur);
        return ResponseEntity.created(new URI("/api/bailleurs/" + bailleur.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, bailleur.getId().toString()))
            .body(bailleur);
    }
    ...
    public ResponseEntity<Bailleur> updateBailleur(
        ...
        @Valid @RequestBody Bailleur bailleur
        ...
        bailleur = bailleurService.update(bailleur);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bailleur.getId().toString()))
            .body(bailleur);
    }
    ...
    public ResponseEntity<Bailleur> partialUpdateBailleur(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Bailleur bailleur
    ) throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bailleur.getId().toString())
        );
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<Bailleur>> getAllBailleurs(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Bailleurs");
        Page<Bailleur> page = bailleurService.findAll(pageable);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Bailleur> getBailleur(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteBailleur(@PathVariable("id") Long id) {
