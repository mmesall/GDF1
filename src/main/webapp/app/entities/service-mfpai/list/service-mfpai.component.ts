...
import { EntityArrayResponseType, ServiceMFPAIService } from '../service/service-mfpai.service';
...
export class ServiceMFPAIComponent implements OnInit {
  ...
  trackId = (_index: number, item: IServiceMFPAI): number => this.serviceMFPAIService.getServiceMFPAIIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.serviceMFPAIS = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IServiceMFPAI[] | null): IServiceMFPAI[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const serviceMFPAISNew = this.serviceMFPAIS ?? [];
      if (data) {
        for (const d of data) {
          if (serviceMFPAISNew.map(op => op.id).indexOf(d.id) === -1) {
            serviceMFPAISNew.push(d);
          }
        }
      }
      return serviceMFPAISNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.serviceMFPAIService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
