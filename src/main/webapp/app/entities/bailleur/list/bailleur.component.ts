...
import { EntityArrayResponseType, BailleurService } from '../service/bailleur.service';
...
export class BailleurComponent implements OnInit {
  ...
  trackId = (_index: number, item: IBailleur): number => this.bailleurService.getBailleurIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.bailleurs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IBailleur[] | null): IBailleur[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const bailleursNew = this.bailleurs ?? [];
      if (data) {
        for (const d of data) {
          if (bailleursNew.map(op => op.id).indexOf(d.id) === -1) {
            bailleursNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.bailleurService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
