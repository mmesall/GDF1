...
import { EntityArrayResponseType, DemandeurService } from '../service/demandeur.service';
...
export class DemandeurComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDemandeur): number => this.demandeurService.getDemandeurIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.demandeurs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDemandeur[] | null): IDemandeur[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const demandeursNew = this.demandeurs ?? [];
      if (data) {
        for (const d of data) {
          if (demandeursNew.map(op => op.id).indexOf(d.id) === -1) {
            demandeursNew.push(d);
          }
        }
      }
      return demandeursNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.demandeurService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
