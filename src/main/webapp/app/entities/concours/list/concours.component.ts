...
import { EntityArrayResponseType, ConcoursService } from '../service/concours.service';
...
export class ConcoursComponent implements OnInit {
  ...
  trackId = (_index: number, item: IConcours): number => this.concoursService.getConcoursIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.concours = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IConcours[] | null): IConcours[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const concoursNew = this.concours ?? [];
      if (data) {
        for (const d of data) {
          if (concoursNew.map(op => op.id).indexOf(d.id) === -1) {
            concoursNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.concoursService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
