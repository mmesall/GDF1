...
import { EntityArrayResponseType, FormationInitialeService } from '../service/formation-initiale.service';
...
@Component({
  ...
  styleUrl: './formation-initiale.component.scss',
  imports: [
    RouterModule,
    FormsModule,
    SharedModule,
...
export class FormationInitialeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFormationInitiale): number => this.formationInitialeService.getFormationInitialeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.formationInitiales = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFormationInitiale[] | null): IFormationInitiale[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const formationInitialesNew = this.formationInitiales ?? [];
      if (data) {
        for (const d of data) {
          if (formationInitialesNew.map(op => op.id).indexOf(d.id) === -1) {
            formationInitialesNew.push(d);
          }
        }
      }
      return formationInitialesNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.formationInitialeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
