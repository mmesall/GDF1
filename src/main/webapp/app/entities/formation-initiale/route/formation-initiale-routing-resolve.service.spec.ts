...
import formationInitialeResolve from './formation-initiale-routing-resolve.service';

describe('FormationInitiale routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: FormationInitialeService;
  let resultFormationInitiale: IFormationInitiale | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IFormationInitiale returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        formationInitialeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultFormationInitiale).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IFormationInitiale>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
