...
export class ExperienceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IExperience): number => this.experienceService.getExperienceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.experiences = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IExperience[] | null): IExperience[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const experiencesNew = this.experiences ?? [];
      if (data) {
        for (const d of data) {
          if (experiencesNew.map(op => op.id).indexOf(d.id) === -1) {
            experiencesNew.push(d);
          }
        }
      }
      return experiencesNew;
    }
    return data ?? [];
  }
