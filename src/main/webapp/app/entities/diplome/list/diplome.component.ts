...
import { EntityArrayResponseType, DiplomeService } from '../service/diplome.service';
...
export class DiplomeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDiplome): number => this.diplomeService.getDiplomeIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IDiplome[] | null): IDiplome[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const diplomesNew = this.diplomes ?? [];
      if (data) {
        for (const d of data) {
          if (diplomesNew.map(op => op.id).indexOf(d.id) === -1) {
            diplomesNew.push(d);
          }
        }
      }
